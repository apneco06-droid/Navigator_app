...
async function generateTexasH1010Pdf(intake, matches) {
    ...
    fillH1010Page1(pages[3], intake, matches, font, boldFont);
    fillH1010Page2(pages[4], intake, font, boldFont);
...
function fillH1010Page16(page, intake, font, boldFont) {
    ...
    drawText(page, intake.interviewLanguage || (intake.language === "es" ? "Spanish" : "English"), 379, 277, 12, font, black, 245);
    if (intake.preferredContactMethod === "phone") {
        drawMark(page, 316, 190, boldFont);
        drawText(page, formatPhone(intake.phone), 510, 209, 12, font, black, 200);
    }
    if (intake.preferredContactMethod === "text") {
        drawMark(page, 316, 141, boldFont);
        drawText(page, formatPhone(intake.cellPhone || intake.phone), 510, 154, 12, font, black, 150);
    }
    if (intake.preferredContactMethod === "email") {
        drawMark(page, 316, 93, boldFont);
        drawText(page, intake.email, 466, 107, 11, font, black, 260);
    }
    drawText(page, formatSsn(intake.socialSecurityNumber), 72, 8, 11, font, black);
}
function fillH1010MrPage1(page, intake, font, boldFont) {
    const black = (0, pdf_lib_1.rgb)(0.08, 0.08, 0.08);
    drawText(page, intake.firstName, 154, 424, 11, font, black, 115);
    drawText(page, intake.middleName, 275, 424, 11, font, black, 105);
    drawText(page, intake.lastName, 429, 424, 11, font, black, 120);
    drawText(page, intake.spouseName, 161, 375, 11, font, black, 360);
    drawYesNo(page, intake.plansToFileTaxes === "yes", 520, 321, 559, 321, boldFont);
    drawYesNo(page, intake.filesJointly === "yes", 520, 283, 559, 283, boldFont);
    drawYesNo(page, intake.claimsDependents === "yes", 520, 247, 559, 247, boldFont);
    drawText(page, intake.dependentNames, 165, 225, 11, font, black, 380);
    drawYesNo(page, intake.claimedAsDependent === "yes", 520, 173, 559, 173, boldFont);
    drawText(page, intake.taxFilerName, 166, 151, 11, font, black, 235);
    drawText(page, intake.taxRelationship, 440, 151, 10.5, font, black, 125);
}
